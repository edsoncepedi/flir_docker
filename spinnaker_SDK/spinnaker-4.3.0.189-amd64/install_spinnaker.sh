...
if [ ! -z "$EXISTING_VERSION" ]; then
    echo "ERROR: An existing Spinnaker installation has been detected (Version: $EXISTING_VERSION)." >&2
    echo "Please uninstall the current version before proceeding with this installation." >&2
    echo "" >&2
    echo "To uninstall, run:" >&2
    echo "  sudo sh remove_spinnaker.sh" >&2
    echo "" >&2
    exit 1
fi
